...
def infer_conditionally(model, tfidf_model, tokenizer, remove_duplicate_phrases=True):
    while True:
        ...
        top_k = input('\nHow many words should we choose from at each step (token/word)? ')
        try:
            top_k = int(top_k)
        except ValueError:
        ...
        variations = 1
        if top_k > 1:
            variations = input('\nHow many variations of commentary for this verse would you like to see? ')
            try:
        ...
        for i in range(variations):
            ...
            if re.search(r'[?.!]\s+(?=[A-Z"])', utterance):
                ...
                print('INFERENCE {} >'.format(i + 1), '<censored>\n')
                continue
...
if __name__ == '__main__':
    model = torch.load('../modeldata/model_0.08020604.pt').to('cuda')
